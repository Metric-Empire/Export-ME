...
class N_PT_Viewport(Panel):
    bl_space_type = "VIEW_3D"
    bl_region_type = "UI"
    bl_label = "Viewport"
    bl_category = "Metric"
    bl_idname = "N_PT_Viewport"
    # bl_options = {'DEFAULT_CLOSED'}

    @classmethod
    def poll(cls, context):
        prefs = context.preferences.addons[base_package].preferences
        clpe = prefs.viewport_panel_enable
        return clpe
    ...
    def draw(self, context):
        ...
        row.operator("spaceview.shadow", text="Shadow")

        row = layout.row(align=True)
        col = row.column(align=True)
        row.operator("spaceview.cavity", text="Cavity")

        col = layout.column(align=True)
        row = layout.row(align=True)
        col.label(text="Noraml:")
        row.operator("spaceview.show_backface", text="Face Orientation")

        col = layout.column(align=True)
        row = layout.row(align=True)
        col.label(text="Clip Distance:")
        row.operator("spaceview.clip_distance_close", text="0.01")
        row.operator("spaceview.clip_distance_mid", text="0.1")
        row.operator("spaceview.clip_distance_far", text="1")

        col = layout.column(align=True)
        row = layout.row(align=True)
        col.label(text="Console Window:")
        row.operator("spaceview.terminal", icon="CONSOLE", text="Terminal")
        row.operator("spaceview.log", icon="INFO", text="Log")
